...
export function AdminInventoryPage() {
  ...
  return <main className="page admin-page"><AppHeader title="Залишки" eyebrow="Склад"/><SearchInput value={search} onChange={setSearch} placeholder="Пошук товару або SKU"/><p className="section-note admin-cart-note">Кошик не резервує товар. Зменшення залишку може позначити позиції користувачів як проблемні, але не видалить їх автоматично.</p>{message?<div className="form-message">{message}</div>:null}<div className="inventory-list">{query.data?.items.flatMap((product)=>product.variants.map((variant)=><InventoryLine key={variant.id} productName={product.name} variant={variant} onMessage={setMessage}/>))}</div></main>;
}
