...
export function ProductPage() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (cart) => { client.setQueryData(['cart'], cart); setAddedOpen(true); notifyHaptic('success'); },
    onError: (value) => { setError(value instanceof ApiError ? value.message : 'Не вдалося додати товар'); notifyHaptic('error'); },
  });
  ...
  return <main className="page product-page"><div className="product-topbar"><IconButton onClick={() => navigate(-1)} aria-label="Назад"><ArrowLeftIcon/></IconButton><span>{product.brand.name}</span><FavoriteButton productId={product.id} initial={product.is_favorite}/></div><div className="product-gallery">{visibleMedia.length ? visibleMedia.map((media) => <figure key={media.id}><img src={media.url} alt={media.alt_text || product.name}/></figure>) : <figure className="product-gallery__fallback">BW</figure>}<div className="product-gallery__count">{Math.max(visibleMedia.length,1)} фото</div></div><section className="product-info"><div className="product-info__badges">{product.is_new ? <NewBadge/> : null}<SaleBadge value={selectedVariant?.discount_percent ?? product.discount_percent}/></div><p className="product-info__brand">{product.brand.name}</p><h1>{product.name}</h1><p className="product-info__model">Модель {product.model_code}</p><Price price={currentPrice} compareAt={currentCompare} currency={product.currency}/>{product.short_description ? <p className="product-info__lead">{product.short_description}</p> : null}<div className={!selectedColor || !selectedSize ? 'variant-selection variant-selection--attention' : 'variant-selection'}><ColorSelector colors={product.colors} selected={selectedColor} onSelect={(code) => { haptic(); setSelectedColor(code); }}/><SizeSelector sizes={product.sizes} variants={product.variants} selectedColor={selectedColor} selectedSize={selectedSize} onSelect={(code) => { haptic(); setSelectedSize(code); }}/></div>{selectedVariant ? <div className={`availability-note ${selectedVariant.is_available ? 'availability-note--yes' : ''}`}><span/>{selectedVariant.is_available ? `У наявності: ${selectedVariant.available_quantity}` : 'Цей варіант тимчасово відсутній'}</div> : <p className="selection-note">Оберіть колір і розмір, щоб додати конкретний варіант.</p>}<AddToCartButton enabled={canAdd} loading={addMutation.isPending} onClick={() => selectedVariant && addMutation.mutate(selectedVariant.id)}/><p className="cart-reservation-note">Товар не резервується до оформлення замовлення.</p><div className="delivery-preview"><BoxIcon/><div><strong>Доставка — preview</strong><span>Умови та строки з’являться разом із checkout.</span></div></div><details className="product-detail-block" open><summary>Опис <ChevronDownIcon size={19}/></summary><p>{product.description || 'Опис уточнюється.'}</p></details><details className="product-detail-block"><summary>Матеріал <ChevronDownIcon size={19}/></summary><p>{product.material || 'Інформація уточнюється.'}</p></details><details className="product-detail-block"><summary>Догляд <ChevronDownIcon size={19}/></summary><p>{product.care_instructions || 'Дотримуйтеся рекомендацій на етикетці.'}</p></details></section><AddedToCartSheet open={addedOpen} onClose={() => setAddedOpen(false)}/><MutationErrorToast message={error} onClose={() => setError(null)}/></main>;
}
