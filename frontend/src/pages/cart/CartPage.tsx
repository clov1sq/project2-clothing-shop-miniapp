...
export function CartPage() {
  ...
  return <main className="page cart-page"><header className="commerce-page-header"><div><p className="page-kicker">Ваші покупки</p><h1>Кошик</h1></div><button type="button" className="clear-cart-button" onClick={() => setClearOpen(true)}><TrashIcon size={18}/>Очистити</button></header><div className="cart-layout"><section className="cart-items">{cart.items.map((item) => <CartItemCard key={item.id} item={item} busy={update.isPending || remove.isPending || refresh.isPending} onQuantity={(quantity) => update.mutate({id:item.id,quantity})} onRemove={() => remove.mutate(item.id)} onAcceptPrice={() => refresh.mutate()}/>)}</section><CartSummary cart={cart}/></div><ClearCartDialog open={clearOpen} loading={clear.isPending} onClose={() => setClearOpen(false)} onConfirm={() => clear.mutate()}/><MutationErrorToast message={error} onClose={() => setError(null)}/></main>;
}
