...
export function CartSummary({ cart }: { cart: CartData }) {
  return <section className="cart-summary"><h2>Підсумок</h2><dl><div><dt>Товарів</dt><dd>{cart.total_quantity}</dd></div><div><dt>Вартість</dt><dd>{formatPrice(cart.subtotal, cart.currency)}</dd></div>{Number(cart.discount_total) > 0 ? <div className="cart-summary__discount"><dt>Знижка</dt><dd>− {formatPrice(cart.discount_total, cart.currency)}</dd></div> : null}<div className="cart-summary__total"><dt>Разом</dt><dd>{formatPrice(cart.grand_total, cart.currency)}</dd></div></dl><p className="cart-summary__reserve">Товар не резервується до оформлення замовлення.</p>{cart.has_issues ? <p className="cart-summary__issue">Виправте проблемні позиції перед майбутнім оформленням.</p> : null}<Button full disabled>Оформлення буде доступне в наступному оновленні</Button></section>;
}
